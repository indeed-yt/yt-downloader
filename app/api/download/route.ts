...
export async function GET(req: Request) {
  try {
    ...
    let selected: YoutubeFormat | undefined = itag ? formats.find((f: YoutubeFormat) => String(f.itag) === String(itag)) : undefined;
    if (!selected) {
    ...
    const parseCodecs = (fmt: YoutubeFormat) => (fmt.codecs || (fmt.mimeType ? fmt.mimeType.split('codecs="')[1]?.split('"')[0] : '')) || '';
    ...
    let chosenAudio: YoutubeFormat | undefined = audioCandidates.find((a: YoutubeFormat) => {
